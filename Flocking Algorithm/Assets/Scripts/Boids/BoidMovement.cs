...
public class BoidMovement : MonoBehaviour
{
    ...
    public LayerMask[] AvoidanceFilters;
    ...
    Vector3 ObstacleAvoidance(Vector3 InHeading)
    {
        ...
        if (Physics.Raycast(transform.position, InHeading, out Hit, ObstacleDistance))
        {
            var OtherLayer = (1 << Hit.transform.gameObject.layer);

            if (!AvoidanceFilters.Contains(OtherLayer))
            {
